...
def data_quality_analysis():
    ...
    with col2:
        ...
        st.metric(
            ...
            delta=(
                f"{validation_accuracy - 90:.1f}%" if validation_accuracy > 90 else None
            ),
        )
    ...
    quality_data = [
        {
            ...
            "Target": "≥90",
            "Status": "✅" if overall_score >= 90 else "⚠️",
        },
        {
            "Metric": "Validation Accuracy",
            "Value": f"{validation_accuracy:.1f}%",
            "Target": "≥95%",
            "Status": "✅" if validation_accuracy >= 95 else "⚠️",
        },
        {
        ...
        {
            "Metric": "Timeliness",
            "Value": f"{timeliness:.1f}%",
            "Target": "≥85%",
            "Status": "✅" if timeliness >= 85 else "⚠️",
        },
        {
            "Metric": "Consistency",
            "Value": f"{consistency:.1f}%",
            "Target": "≥95%",
            "Status": "✅" if consistency >= 95 else "⚠️",
        },
    ]
    ...
    if completeness < 90:
        recommendations.append(
            "📝 Enhance data collection processes to improve completeness"
        )
    if timeliness < 85:
        recommendations.append(
            "⚡ Optimize data synchronization frequency and performance"
        )
    if consistency < 95:
    ...
    with st.expander("📈 Quality Trends (Coming Soon)"):
        st.info(
            "Quality trend analysis and historical tracking will be available in future releases."
        )
